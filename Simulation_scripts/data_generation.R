...
data.p.210<- function(n){
  ...
  for(k in 1:Nk){
    ...
    ################################# y ############################
    foo7<- get(paste0("x",temp[1]+6,".s"))
    foo8<- get(paste0("x",temp[2]+6,".s"))
    
    p.y<-exp(beta0.y + beta1.y*foo7 + beta2.y*foo8)/(1+exp(beta0.y + beta1.y*foo7 + beta2.y*foo8))
    Y.disc<-vapply(p.y, rbinom, FUN.VALUE = 0,n=1, size=1)
    assign(paste0("Y.disc.",k),Y.disc)
  ...
  for(k in 1:Nk){
    y.d<- get(paste0("Y.disc.",k))
    Y.n<- cbind(Y.n, y.d)
  }
  colnames(Y.n)[(20*Nk+1):(21*Nk)]<- paste0("y",1:Nk)
  file.name<- paste0("data_n",n,"_p",p,"_rep",rep,".RData")
  save(Y.n, file=file.name)
  }
} ########## data generation for 200 continuous and 10 binary nodes

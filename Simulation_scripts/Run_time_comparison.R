...
for(i in p)
{
  load(paste0("data/data_n_500_p_",i,".RData"))
  
  load(paste0("adjn",i,".RData"))
  
  true.ske=(adjn+t(adjn))>0+0
  
  print(paste0("running ",i," ","true.skel = ",sum(true.ske)))
  
  print(dim(Y.n))
  
  colnames(Y.n)<- paste0("X",1:i)

  init<-proc.time()
  
  mdag.ad<- mDAG(data=Y.n, type=rep("g",i), level=rep(1,i), nperm=10000)
  
  time.md<-(proc.time() - init)[3] 
  
  time.md.vec<-c(time.md.vec,time.md)
}